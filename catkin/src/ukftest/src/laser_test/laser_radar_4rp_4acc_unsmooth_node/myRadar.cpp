...
void OpenRadar::DrawRadarData()
{
	...
	{
		time_cnt = -1;
		ofstream file("/mnt/iarc/scan.txt");
		for (int i = 0; i < RadarDataCnt;i++)
		{
			x = (int)(RadarRho[i]*cos(RadarTheta[i])/DisplayRatio);
			y = (int)(-RadarRho[i]*sin(RadarTheta[i])/DisplayRatio);
			file<< x<< " "<< y<< endl;
	...
	//pt2.x = DisplayDx+line_length*cos(-(0 + skip_bin_idx * polarH_resolution)* M_PI/180 ); 	
	//pt2.y = DisplayDy+line_length*sin(-(0 + skip_bin_idx * polarH_resolution)* M_PI/180 ); 
	//cvLine(RadarImage, pt1, pt2, CV_RGB(0,255,0),1,8,0);

	//pt2.x = DisplayDx+line_length*cos(-(0 + (polarH_length-skip_bin_idx) * polarH_resolution)* M_PI/180 ); 
	//pt2.y = DisplayDy+line_length*sin(-(0 + (polarH_length-skip_bin_idx) * polarH_resolution)* M_PI/180 ); 
	...
	for (int i=0; i<polarH_length;i++)
	{
		angle = (0+i*polarH_resolution)*M_PI/180;
		line_length2 = H[i]/10;
		pt2.x = DisplayDx+line_length2*cos(angle); 
		pt2.y = DisplayDy+line_length2*sin(angle);
